...
$url = [regex]::match($currentversion.Content, "blockquote class=`"well`".*?href=`"(.*?)`"", [Text.RegularExpressions.RegexOptions]::Singleline).Groups[1].Value
...
$citemplate = Get-Content ".\templates\burnawarefree\chocolateyInstall.ps1.template" -raw

$citemplate = $citemplate -replace "%hash%", "$filehash"
$citemplate = $citemplate -replace "%downloadurl%", "$url"

$citemplate | Out-File ".\burnawarefree\tools\chocolateyInstall.ps1"

Write-Host "Processing nuspec file..."
$nstemplate = Get-Content ".\templates\burnawarefree\burnawarefree.nuspec.template" -raw
...
Set-Location -Path ..
Remove-Item -path .\burnawarefree -recurse
